...
String group = "SamirGroup";   // UPDATE THIS with an unique name for your group
...
void onMqttMessage(int messageSize) {
  ...
  while (mqttClient.available()) {
    readString += (char)mqttClient.read();
    Serial.println(readString);
//    Serial.print((char)mqttClient.read());
  }
  Serial.println("Checking the message category");
  if (readString.length() >0) {
      Serial.print("Read string is :");
      Serial.println(readString);
  
      if (readString == "ON")     
      {
        Serial.print("Finished reading from Serial message is: ");
        Serial.println(readString);
